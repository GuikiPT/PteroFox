  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (process.env.DEBUG) {
        console.log(
          colors.cyan(
            `[DEBUG] Bot Latency: ${botLatency}ms | API Latency: ${apiLatency}ms`
          )
        );
      }
    } catch (error) {
...
async function handleCommandError(interaction, action, error, isPrivate) {
  const errorMessage = `❌ An error occurred while executing this command while ${action}. Please try again later.`;
  ...
  try {
    ...
    console.error(
      colors.red(
        `Failed to send error message: ${replyError.stack || replyError}`
      )
    );
  }
}
